...
class ExoticLinearAmortizer():
    ...
    # Array Cycle Anchor Date Of Interest Payment :: Same like IPANX but as array
    array_cycle_anchor_date_of_interest_payment : Timestamp[]
    
    # Array Cycle Of Interest Payment :: Same like IPCL but as array
    array_cycle_of_interest_payment : Cycle[]
    ...
    cycle_of_interest_calculation_base : Cycle
    ...
    # Cycle Point Of Interest Payment :: Usually, interest is paid at the end of each IPCL which corresponds to a IPPNT value of E which is also the default. If interest payment occurs at the beginning of the cycle, the value is B.
    cycle_point_of_interest_payment : enums.CyclePointOfInterestPayment
    ...
    # Array Cycle Anchor Date Of Principal Redemption :: Same like PRANX but as array
    array_cycle_anchor_date_of_principal_redemption : Timestamp[]
    
    # Array Cycle Of Principal Redemption :: Same like PRCL but as array
    array_cycle_of_principal_redemption : Cycle[]
    
    # Array Next Principal Redemption Payment :: Same like PRNXT but as array
    array_next_principal_redemption_payment : Real[]
    
    # Array Increase Decrease :: Indicates whether a certain PRNXT element in ARPRNX increases the principal (NT) or decreases it.Applies only for ANX, NAX, LAX Maturity CTs. For all other Maturity CTs the first principal payment is always in the opposite direction of all other (following) payments.
    array_increase_decrease : Enum[]
    ...
    # Array Cycle Anchor Date Of Rate Reset :: Same like RRANX but as array
    array_cycle_anchor_date_of_rate_reset : Timestamp[]
    
    # Array Cycle Of Rate Reset :: Same like RRCL but as array
    array_cycle_of_rate_reset : Cycle[]
    
    # Array Rate :: For array-type rate reset schedules, this attribute represents either an interest rate (corresponding to IPNR) or a spread (corresponding to RRSP). Which case applies depends on the attribute ARFIXVAR: if ARFIXVAR=FIX then it represents the new IPNR and if ARFIXVAR=VAR then the applicable RRSP.
    array_rate : Real[]
    
    # Array Fixed Variable :: For array-type rate reset schedules, this attributes defines the meaning of ARRATE.
    array_fixed_variable : Enum[]
