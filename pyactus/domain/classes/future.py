...
class Future():
    ...
    # Clearing House :: Indicates wheter CRID takes a clearing house function or not. In other word, whether CRID receive margins (MRIM, MRVM).
    clearing_house : enums.ClearingHouse
    
    # Initial Margin :: Margin to cover losses which may be incurred as a result of market fluctuations. Upon contract closing or maturity, the MRIM is reimbursed.
    initial_margin : float
    
    # Maintenance Margin Lower Bound :: Defines the lower bound of the Maintenance Margin. If MRVM falls below MRMML, then capital must be added to reach the original MRIM.
    maintenance_margin_lower_bound : float
    
    # Maintenance Margin Upper Bound :: Defines the upper bound of the Maintenance Margin. If MRVM falls above MRMMU, then capital is refunded to reach the original MRIM.
    maintenance_margin_upper_bound : float
    
    # Cycle Anchor Date Of Margining :: Date from which the margin call date schedule is calculated according to the cycle length. The first margin call event takes place on this anchor.
    ...
    # Futures Price :: The price the counterparties agreed upon at which the underlying contract (of a FUTUR) is exchanged/settled at STD. Quoting is different for different types of underlyings: Fixed Income = in percentage, all others in nominal terms.
    futures_price : float
